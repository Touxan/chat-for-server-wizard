...
import { PlusCircle, ChevronDown, ChevronRight, FolderIcon, Terminal } from "lucide-react";
...
const Sidebar = forwardRef<HTMLDivElement, SidebarProps>(({ isOpen }, ref) => {
  return (
    <div
      ...
      } fixed top-0 left-0 h-full bg-black shadow-xl w-80 transition-transform duration-300 ease-in-out z-40 pt-16 text-[#0f0] font-mono`}
    >
      <div className="p-4">
        <Button className="w-full bg-black hover:bg-black/90 border border-[#0f0]/30 shadow-md mb-4 py-5 text-[#0f0] font-mono rounded-sm">
          <PlusCircle className="mr-2 h-4 w-4" />
          new_chat
        </Button>
        <Separator className="my-4 bg-[#0f0]/30" />
        
        <div className="mb-6">
          <h2 className="text-xs uppercase font-mono text-[#0f0] mb-3 tracking-wider">~/chat_logs</h2>
          <ScrollArea className="h-[calc(100vh-180px)]">
            <div className="space-y-2">
              {groupedChats.map((group, index) => (
                <Collapsible key={index} defaultOpen={index < 2}>
                  <div className="flex items-center mb-1">
                    <CollapsibleTrigger className="flex items-center w-full text-sm text-[#0f0]/80 hover:text-[#0f0]">
                      <span className="flex items-center w-full">
                        ...
                        <FolderIcon className="h-4 w-4 mr-2 text-[#0f0]" />
                    ...
                    <div className="ml-6 space-y-1">
                      {group.chats.map((chat) => (
                        <Button
                          key={chat.id}
                          variant="ghost"
                          className="w-full justify-start text-left font-mono text-sm text-[#0f0]/80 hover:bg-[#0f0]/10 hover:text-[#0f0] rounded-sm"
                        >
                          <Terminal className="mr-2 h-4 w-4 text-[#0f0]" />
                          <div className="flex-1 overflow-hidden">
                            <div className="truncate">{chat.title}</div>
                            <p className="text-xs text-[#0f0]/60">{chat.date}</p>
                          </div>
                        </Button>
                      ))}
                    </div>
